...
function	verify(prefix, nginxValue, webservValue, print)
{
	if (print == false)
		return (nginxValue === webservValue);
...
function	compareStatus(nginxResponse, webservResponse, print)
{
	//status code + text
	const	nginxStatus = getStatus(nginxResponse);
	const	webservStatus = getStatus(webservResponse);

	return (verify("status : ", nginxStatus , webservStatus, print));
}

function	compareRedirection(nginxResponse, webservResponse, print)
{
	//redirection
	...
	if (nginxRedirection !== null && webservRedirection !== null)
			return (verify("redirection : ", nginxRedirection, webservRedirection, print));
	return (true);
}

async function	compareBody(nginxResponse, webservResponse, print)
{
	//body
	...
	{
		return (verify("body : ", nginxBody, webservBody, print));
	}
	return (true);
}

function	compareRequestEffect(target, nginxResponse, webservResponse, print)
{
		...
		|| (nginxResponse.status == 204 && webservResponse.status == 204))
	{
		...
		return (compareGoodRequests(target, "GET", null, [], print));
	}
	return (true);
}

async function	compareRequests(target, nginxResponse, webservResponse, print)
{
	let	succeed = true;

	succeed = compareStatus(nginxResponse, webservResponse, print) && succeed;
	succeed = compareRedirection(nginxResponse, webservResponse, print) && succeed;
	succeed = await compareBody(nginxResponse, webservResponse, print) && succeed;
	succeed = await compareRequestEffect(target, nginxResponse, webservResponse, print) && succeed;
	return (succeed);
}

export async function	compareGoodRequests(target, method, body, headers, print)
{
	if (print == true)
	...
	return (compareRequests(target, nginxResponse, webservResponse, print));
}

export async function	compareBadRequests(message, target, print)
{
	if (print == true)
...
export async function	compareBadRequestWithValues(message, statusCode, statusText, print)
{
	const webservResponse = await makeRawRequest(webservHost, webservPort, message);
	const expectedResponse = {
		status: statusCode,
	...
	return (compareStatus(expectedResponse, webservResponse, print));
}
