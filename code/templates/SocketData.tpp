 ...
 */
template <typename T>
SocketData::SocketData(int fd, T &data, void (&callback)(const SocketData& socketData, T *data, uint32_t events)) :
	_fd(fd),
	_data(&data),
	_callback(reinterpret_cast<void (&)(const SocketData& socketData, void *data, uint32_t events)>(callback)),
	_iterator(),
